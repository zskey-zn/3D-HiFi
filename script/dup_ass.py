# -*- coding: utf-8 -*-
"""
V1 Created on Tue Sep 16 10:14:35 2025
V2 Fixed on Wed Aug 12 11:55:14 CST 2026
@author: zhengshang@frasergen.com zhengshang-zn@qq.com
脚本说明：在原来聚类的结果里加入坍塌的contig信息
# 使用方法：
python dup_ass.py -a test.p_utg.0.asm -c test.p_utg.cprops -l collapsed.contig.list -o contig.dup.0.assembly
"""
...
def main():
    ...
    collapsed_ctg = defaultdict(list)
    with open(args.collapsed_list, 'r') as f:
        for line in f:
            tmp=line.split()
            ctg_id=tmp.pop(0)
            collapsed_ctg[ctg_id]=tmp
    ...
    for hic_id, contig_id in raw_hic_dict.items():
        if contig_id in collapsed_ctg.keys():
            count += 1
            dup_hic_dict[contig_id] = str(count)
            dup_len_dict[contig_id] = len_dict[contig_id]
            for i in  collapsed_ctg[contig_id]:
                count += 1
                dup_hic_dict[f"{contig_id}_{i}"] = str(count)
                dup_len_dict[f"{contig_id}_{i}"] = len_dict[contig_id]
        else:
            count += 1
            dup_hic_dict[contig_id] = str(count)
            dup_len_dict[contig_id] = len_dict[contig_id]
